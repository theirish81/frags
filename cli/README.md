...
A command-line interface for [Frags](https://github.com/theirish81/frags) to process session files using AI models from Google Gemini or Ollama.
...
-   `AI_ENGINE`: The AI engine to use. Can be `gemini` or `ollama`. If not set, the tool will try to guess based on the
...
```
# The AI engine to use: "gemini" or "ollama"
...
## Download
You can find pre-built binaries for Windows, Linux, and macOS on the [Releases page](https://github.com/theirish81/frags/releases).
Remember these binaries are not signed, so you may need to disable your security settings to run them.
...
Render a YAML or JSON data file using a Go template. This is useful for re-formatting previous outputs without re-running the AI session.
